...
export function EndpointList() {
  ...
  const [activeEndpoints, setActiveEndpoints] = useState([...endpointData.map((_, index) => index)]);
        ...
        return (
          <div key={data.controllerName} className="flex flex-col gap-2">
            <ControllerButton onClick={() => onClickController(index)}>
              ...
              <Icon icons={activeEndpoints.includes(index) ? ICONS_LIST.ARROW_DROP_DOWN : ICONS_LIST.ARROW_DROP_UP} />
            </ControllerButton>
            {activeEndpoints.includes(index) && (
              <div className="flex flex-col gap-4">
